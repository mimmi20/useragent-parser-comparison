...
final class GenerateReports extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $statementSelectProvider = $this->pdo->prepare('SELECT * FROM `real-provider` WHERE `proId` IN (SELECT DISTINCT `provider_id` FROM `result` WHERE `run` = :runId)');
...
//            }
            ...
            $message = $baseMessage . sprintf(' provider <fg=yellow>%s</> <info>done</info>', $dbResultProvider['proName']);
